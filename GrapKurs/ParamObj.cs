...
namespace GrapKurs
{
    class ParamObj
    {
        ...
        public ParamObj(Point start_point, double bar_len, double bar_diam, double mag_len, double box_width, double cev_len, double targ_ang, int nas, int rings, double sp_ang, double dist)
        {
            ...
            /*Ствол*/
            Tube barrel = new Tube(Start, bar_diam, bar_diam - 1.5, bar_len, Color.Red);//bar_len = 200; bar_diam = 4/2.5
            barrel.Rotate(0, 0, 90, Start);
            barrel.Resave();
            foreach (Triangle item in barrel.polygons)
                polygs.Add(item);
            /*Магазин*/
            Cylinder magazine = new Cylinder(Start, 4, mag_len, Color.Brown);//mag_len = 180
            ...
            foreach (Triangle item in magazine.polygons)
                polygs.Add(item);
            /*Цевьё*/
            Cylinder cev = new Cylinder(Start, 5.5, cev_len, Color.RoyalBlue);//cev_len = 170
            ...
            foreach (Triangle item in cev.polygons)
                polygs.Add(item);
            /*Затворная коробка*/
            Point box1 = new Point(Start.x, Start.y + 10, Start.z + box_width / 2); //box_width = 6
            Point box2 = new Point(Start.x + 80, Start.y - (3 + 5), Start.z - box_width / 2);
            Box box = new Box(box1, box2, Color.Beige);
            box.Resave();
            foreach (Triangle item in box.polygons)
                polygs.Add(item);
            /*Целик*/
            Point celStart = new Point(box1.x - dist, box1.y - 2, box1.z);//dist = 30
